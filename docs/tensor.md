...
which can be checked by `#ifdef LONEXGRAD_CUDA`.
...
// In function:
//         lonexgrad::Tensor lonexgrad::add(const Tensor&, const Tensor&)
// 
// lonexgrad Error: Expected inputs to be on same device, given devices cuda:0, cuda:1
