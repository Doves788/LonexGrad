...
#include <lonexgrad/nn/lonex_gru.h>
...
auto read_lang_file(
    ...
    while (std::getline(file, line)) {
        ...
        if (s_pair.size() != 2) {
            LONEXGRAD_EXCEPTION("Expected line to be split by tab character");
        }
...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        LONEXGRAD_EXCEPTION("Usage: ./translation_example file_dir input_lang output_lang");
    }
