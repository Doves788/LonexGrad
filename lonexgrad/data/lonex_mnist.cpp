...
#include <lonexgrad/data/lonex_mnist.h>
...
auto read_mnist(const std::string &image_path, const std::string &label_path) -> std::tuple<Tensor, Tensor, int> {
    ...
    if (image_file.fail()) {
        LONEXGRAD_EXCEPTION(std::format("Error trying to read image file {:s}", image_path));
    }
    if (label_file.fail()) {
        LONEXGRAD_EXCEPTION(std::format("Error trying to read label file {:s}", label_path));
    }
    ...
    if (magic != 2051) {
        LONEXGRAD_EXCEPTION(std::format("Incorrect image file magic value {:d}, expected {:d}", magic, MAGIC_VAL1));
    }

    label_file.read(reinterpret_cast<char *>(&magic), 4);    // NOLINT(*-reinterpret-cast)
    magic = swap_endian(magic);
    if (magic != MAGIC_VAL2) {
        LONEXGRAD_EXCEPTION(std::format("Incorrect image file magic value {:d}, expected {:d}", magic, MAGIC_VAL2));
    }
