...
#include <lonexgrad/nn/lonex_batchnorm.h>
...
BatchNorm1d::BatchNorm1d(int num_features, const BatchNormOptions &options, ScalarType dtype, Device device)
    : gamma(std::make_shared<Tensor>(ones({1, num_features}, dtype, device, options.affine))),
      ...
      options_(options) {
    if (num_features <= 0) {
        LONEXGRAD_EXCEPTION(std::format("Expected num_features > 0, given num_features={:d}", num_features));
    }
...
auto BatchNorm1d::forward(const Tensor &x) -> Tensor {
    if (x.dim() != 2 && x.dim() != 3) {
        LONEXGRAD_EXCEPTION(std::format("Expected input to have 2 or 3 dimensions, given shape {}", x.shape()));
    }
    if (x.size(1) != num_features_) {
        LONEXGRAD_EXCEPTION(std::format("Expected size of input at dim 1 to be {}, given shape {}", num_features_, x.shape()));
    }
...
auto BatchNorm2d::forward(const Tensor &x) -> Tensor {
    if (x.dim() != 4) {
        LONEXGRAD_EXCEPTION(std::format("Expected input to have 4 dimensions, given shape {}", x.shape()));
    }
    if (x.size(1) != num_features_) {
        LONEXGRAD_EXCEPTION(std::format("Expected size of input at dim 1 to be {}, given shape {}", num_features_, x.shape()));
    }
