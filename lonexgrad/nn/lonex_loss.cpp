...
auto cross_entropy_loss(
    ...
    if (!is_integral_dtype(target_indices.dtype())) {
        LONEXGRAD_EXCEPTION(std::format("Expected indices to be of integral dtype, given {:s}", target_indices.dtype()));
    }
    ...
    case ReductionMode::sum:
        return loss.sum();
    default:
        LONEXGRAD_EXCEPTION(std::format("Reduction mode {:s} is unsupported for this loss", reduction_mode_to_str(mode)));
    }
    unreachable();
}
