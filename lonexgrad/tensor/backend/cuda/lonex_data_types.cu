...
#include "tensor/backend/cuda/kernel/lonex_init.cuh"
...
DeviceMemory<T>::DeviceMemory(int device_id, std::size_t n)
    ...
    if (malloc_status != cudaSuccess) {
        LONEXGRAD_EXCEPTION("cudaMalloc error: " + std::string(cudaGetErrorString(malloc_status)));
    }
...
DeviceMemory<T>::DeviceMemory(int device_id, const std::vector<std::remove_cv_t<T>> &v)
    ...
    if (memcpy_status != cudaSuccess) {
        LONEXGRAD_EXCEPTION("cudaMemcpy error: " + std::string(cudaGetErrorString(memcpy_status)));
    }
    StorageCUDA::current_bytes_allocated[device_id] += _n * sizeof(T);
    StorageCUDA::total_bytes_allocated[device_id] += _n * sizeof(T);
}
...
DeviceMemory<T>::~DeviceMemory() {
    if (_p) {
        ...
        if (device_status != cudaSuccess) {
            LONEXGRAD_ERROR("cudaMalloc error: " + std::string(cudaGetErrorString(device_status)));
        }
        const auto status = cudaFree(_p);
        if (status != cudaSuccess) {
            LONEXGRAD_ERROR("cudaFree error: " + std::string(cudaGetErrorString(status)));
        }
        _p = nullptr;
        _n = 0;
    }
...
auto DeviceMemory<T>::to_vec() const -> std::vector<std::remove_cv_t<T>> {
    ...
    if (status != cudaSuccess) {
        LONEXGRAD_EXCEPTION("cudaMemcpy error: " + std::string(cudaGetErrorString(status)));
    }
    return v;
}
...
auto DeviceMemory<T>::item(int index) const -> T {
    if (index < 0 || static_cast<std::size_t>(index) > _n) {
        LONEXGRAD_EXCEPTION(std::format("Index {} is out of bounds for storage of size {}", index, _n));
    }
    const auto device_status = cudaSetDevice(_device_id);
    if (device_status != cudaSuccess) {
        LONEXGRAD_EXCEPTION("cudaMalloc error: " + std::string(cudaGetErrorString(device_status)));
