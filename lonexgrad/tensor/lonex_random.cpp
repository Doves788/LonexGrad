...
LONEXGRAD_HOST_DEVICE auto splitmix64(uint64_t seed) noexcept -> uint64_t {
...
LONEXGRAD_HOST_DEVICE auto xorshift64(uint64_t &state) noexcept -> uint64_t {
    uint64_t x = state;
    x ^= x << XOR64_S1;
    x ^= x >> XOR64_S2;
    x ^= x << XOR64_S3;
...
LONEXGRAD_HOST_DEVICE Generator::Generator(uint64_t s, bool is_state)
    : state(is_state ? s : splitmix64(s)) {}

LONEXGRAD_HOST_DEVICE Generator::Generator(uint64_t seed)
    : Generator(seed, false) {}

LONEXGRAD_HOST_DEVICE auto Generator::from_state(uint64_t _state) -> Generator {
    return {_state, true};
}

LONEXGRAD_HOST_DEVICE auto Generator::operator()() noexcept -> uint64_t {
    return xorshift64(state);
}

LONEXGRAD_HOST_DEVICE void Generator::set_state(uint64_t _state) {
    state = _state;
}
