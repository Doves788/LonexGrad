...
#include "autograd/lonex_conv_ops.h"
...
#define CHECK_VALID_TENSOR(t)                           \
    if (!(t).has_storage()) {                           \
        LONEXGRAD_EXCEPTION("Detected uninitialized Tensor."); \
    }
...
#define CHECK_VALID_INDEX_INCLUSIVE(dim, N)                                                   \
    if (dim > N || dim < -N - 1) {                                                            \
        LONEXGRAD_EXCEPTION(std::format("Invalid dim, expected to be in range[{}, {}]", -N - 1, N)); \
    }
...
#define CHECK_EMPTY_SHAPE(shape)            \
    if (shape.numel() == 0) {               \
        LONEXGRAD_EXCEPTION("Given empty shape."); \
    }
...
#define CHECK_SELF_FLOAT()                                                                               \
    if (!is_float_dtype(scalar_type_)) {                                                                 \
        LONEXGRAD_EXCEPTION(std::format("Expected floating point self dtype. Self dtype is {:s}", scalar_type_)); \
    }

// NOLINTNEXTLINE*(-macro-usage)
#define CHECK_INPLACE_AUTOGRAD(t)                                                                       \
    if (autograd::GradMode::is_enabled() && (t).requires_grad()) {                                      \
        LONEXGRAD_EXCEPTION("Performing inplace operation on tensor which requires grad is not yet supported."); \
    }

// NOLINTNEXTLINE*(-macro-usage)
#define DISABLE_BOOL(t, OP)                                                    \
    if ((t).dtype() == kBool) {                                                \
        LONEXGRAD_EXCEPTION(std::format("{:s} on boolean tensors is not supported.", #OP)); \
    }
...
Tensor::Tensor(const std::vector<T> &data, Shape shape, Device device, bool requires_grad)
    ...
    if (data.empty()) {
        LONEXGRAD_EXCEPTION("Cannot make empty tensor");
    }
...
auto Tensor::operator[](const std::vector<indexing::Index> &indices) const -> Tensor {
    CHECK_VALID_TENSOR(*this);
    if (indices.empty()) {
        LONEXGRAD_EXCEPTION("Indexing with an empty list");
    }
...
auto index_mask(const Tensor &self, const Tensor &mask) -> Tensor {
    ...
    if (mask_count == 0) {
        LONEXGRAD_EXCEPTION("Indexing Tensor with false mask will result in empty Tensor");
    }
...
auto index_select(const Tensor &tensor, const std::vector<int> &indices, int dim) -> Tensor {
    CHECK_VALID_TENSOR(tensor);
    if (indices.empty()) {
        LONEXGRAD_EXCEPTION("Cannot index with empty indices list.");
    }
...
auto index_select(const Tensor &tensor, const Tensor &indices, int dim) -> Tensor {
    ...
    if (!is_integral_dtype(indices.dtype())) {
        LONEXGRAD_EXCEPTION(std::format("Indices must be an integral type, given dtype = {:s}", indices.dtype()));
    }
...
auto repeat_interleave(const Tensor &tensor, int repeats, int dim) -> Tensor {
    ...
    dim = indexing_exclusive(dim, tensor.shape());
    if (repeats < 1) {
        LONEXGRAD_EXCEPTION(std::format("Expected repeats > 0, given {:d}", repeats));
    }
...
auto repeat(const Tensor &tensor, const Tensor &repeats) -> Tensor {
    ...
    if (!is_integral_dtype(repeats.dtype())) {
        LONEXGRAD_EXCEPTION(std::format("Indices must be an integral type, given dtype = {:s}", repeats.dtype()));
    }
...
auto gather(const Tensor &tensor, const Tensor &_indices, int dim) -> Tensor {
    ...
    if (!is_integral_dtype(_indices.dtype())) {
        LONEXGRAD_EXCEPTION(std::format("indices scalar type should be integral, given {:s}", _indices.dtype()));
    }
...
void Tensor::add_grad(const Tensor &grad) {
    ...
    if (requires_grad()) {
        ...
        if (grad.shape() != shape_) {
            LONEXGRAD_ERROR(std::format("Grad shape ({:s}) does not match the tensor shape ({:s}).", grad.shape(), shape_));
        }
        if (ctx_->grad) {
...
void Tensor::apply_grad_hook() {
    ...
    if (ctx_->grad_hook) {
        if (!ctx_->grad) {
            LONEXGRAD_EXCEPTION("Grad hook called on tensor without gradient set.");
        }
...
void Tensor::backward(const std::optional<Tensor> &grad, bool retain_graph) {
    CHECK_VALID_TENSOR(*this);
    if (!is_float_dtype(scalar_type_)) {
        LONEXGRAD_EXCEPTION(std::format("Can only call backward on floating point tensors. Given dtype %s", scalar_type_));
    }
    if (!ctx_->requires_grad) {
        LONEXGRAD_EXCEPTION("Calling backward on tensor which does not require a gradient");
    }
    if (!ctx_->grad_func) {
        LONEXGRAD_EXCEPTION("Calling backward on tensor from non-differentiable operation");
    }
    if (grad.has_value()) {
...
auto broadcast_result_shape(const Shape lhs, const Shape rhs) -> Shape {
    ...
    if (!are_broadcastable(lhs, rhs)) {
        LONEXGRAD_EXCEPTION(std::format("Shape {:s} and shape {:s} are not broadcastable.", lhs.to_string(), rhs.to_string()));
    }
...
auto Tensor::broadcast_to(const Shape &shape) const -> Tensor {
    ...
    if (shape.ndim() == 0 || shape.numel() == 0) {
        LONEXGRAD_EXCEPTION("Cannot broadcast to empty shape");
    }
...
auto ones(Shape shape, ScalarType dtype, Device device, bool requires_grad) -> Tensor {
    CHECK_EMPTY_SHAPE(shape);
    CHECK_REQUIRES_GRAD(dtype, requires_grad);
    return full(Scalar(1, dtype), shape, device, requires_grad);
}
...
auto arange(Shape shape, ScalarType dtype, Device device, bool requires_grad) -> Tensor {
    CHECK_EMPTY_SHAPE(shape);
    if (dtype == kBool) {
        LONEXGRAD_EXCEPTION("arange not defined for boolean scalar type.");
    }
    CHECK_REQUIRES_GRAD(dtype, requires_grad);
    const auto N = static_cast<std::size_t>(shape.numel());
...
auto linspace(
    ...
    CHECK_EMPTY_SHAPE(shape);
    if (dtype == kBool) {
        LONEXGRAD_EXCEPTION("linspace not defined for boolean scalar type.");
    }
    CHECK_REQUIRES_GRAD(dtype, requires_grad);
    const auto N = static_cast<std::size_t>(shape.numel());
...
auto eye(int rows, int cols, ScalarType dtype, Device device, bool requires_grad) -> Tensor {
    ...
    if (dtype == kBool) {
        LONEXGRAD_EXCEPTION("eye not defined for boolean scalar type.");
    }
    CHECK_REQUIRES_GRAD(dtype, requires_grad);
    autograd::NoGradGuard guard;
    std::vector<int> indices;
...
auto one_hot(Tensor indices, int num_classes) -> Tensor {
    ...
    if (!is_integral_dtype(indices.dtype())) {
        LONEXGRAD_EXCEPTION(std::format("Expected indices to be of integral dtype, given {:s}", indices.dtype()));
    }
    autograd::NoGradGuard guard;
    ...
    if (indices_min < 0 || indices_max >= num_classes) {
        LONEXGRAD_EXCEPTION(std::format("indices contains a value outside the expected range of [0, {:d})", num_classes));
    }
...
auto cat_runner(const TensorList &tensors, int dim, const std::optional<Tensor> &out) -> Tensor {
    if (tensors.empty()) {
        LONEXGRAD_EXCEPTION("Given empty vector of tensors to concatenate.");
    }
    if (tensors.size() == 1) {
        return tensors[0];
    }
...
auto stack_runner(const TensorList &tensors, int dim, const std::optional<Tensor> &out) -> Tensor {
    ...
    for (const auto &tensor : tensors) {
        expanded_tensors.push_back(tensor.unsqueeze(dim));
    }
...
#define CHECK_UNIFORM(low, high)                                              \
    ...
    if ((high <= low).any()) {                                                \
        LONEXGRAD_EXCEPTION("Expected low < high. At least one element violates this."); \
    }
...
#define CHECK_BERNOULLI(p)                                                                      \
    ...
    if ((p < 0 || p > 1).any()) {                                                               \
        LONEXGRAD_EXCEPTION("Expected p in range [0, 1]. At least one element in p violates this."); \
    }
...
#define CHECK_BINOMIAL(p, num_draws)                                                                           \
    ...
    if ((num_draws < 0).any()) {                                                                               \
        LONEXGRAD_EXCEPTION("Expected num_draws >= 0. At least one element in num_draws violates this."); \
    }
...
#define CHECK_EXPONENTIAL(lambda)                                                       \
    if ((lambda <= 0).any()) {                                                          \
        LONEXGRAD_EXCEPTION("Expected lambda > 0. At least one element in lambda violates this."); \
    }
...
#define CHECK_NORMAL(std)                                                       \
    if ((std <= 0).any()) {                                                     \
        LONEXGRAD_EXCEPTION("Expected std > 0. At least one element in std violates this."); \
    }

// NOLINTNEXTLINE*(-macro-usage)
#define CHECK_CAUCHY(scale)                                                     \
    if ((scale <= 0).any()) {                                                   \
        LONEXGRAD_EXCEPTION("Expected scale > 0. At least one element in scale violates this."); \
    }

// NOLINTNEXTLINE*(-macro-usage)
#define CHECK_WEIBULL(lambda, k)                                                          \
    if ((lambda <= 0 || k <= 0).any()) {                                                  \
        LONEXGRAD_EXCEPTION("Expected both lambda > 0, k > 0. At least one element violates this."); \
    }
...
#define DECLARE_BINARY_INT_OPS(FUNC)                                                                                \
    auto FUNC(const Tensor &lhs, const Tensor &rhs) -> Tensor {                                                     \
        ...
        if (is_float_dtype(lhs.dtype()) || is_float_dtype(rhs.dtype())) {                                           \
            LONEXGRAD_EXCEPTION("Expects arguments to be of integral dtype");                                             \
        }                                                                                                           \
        const auto promoted_type = promote_types(lhs.dtype(), rhs.dtype());                                         \
...
#define DECLARE_BINARY_SHIFT_OPS(FUNC)                                                \
    auto FUNC(const Tensor &lhs, const Tensor &rhs) -> Tensor {                       \
        ...
        if (is_float_dtype(lhs.dtype()) || is_float_dtype(rhs.dtype())) {             \
            LONEXGRAD_EXCEPTION("Bitwise shift expects arguments to be of integral dtype");  \
        }                                                                             \
...
auto matmul(const Tensor &_lhs, const Tensor &_rhs) -> Tensor {
    ...
    if (promoted_type == kBool) {
        LONEXGRAD_EXCEPTION(std::format("matmul does not support boolean tensors. Given {:s}", concat_dtype_str(_lhs, _rhs)));
    }
    ...
    if (lhs_shape.ndim() == 1 && rhs_shape.ndim() == 1) {    
        ...
        return autograd::TensorBatchedMatmul::apply(
            ...
            .flatten();
    } else if (lhs_shape.ndim() == 1 && rhs_shape.ndim() == 2) {    
        if (lhs_shape[0] != rhs_shape[0]) {
            LONEXGRAD_EXCEPTION(
                std::format(
    ...
    } else if (lhs_shape.ndim() == 2 && rhs_shape.ndim() == 1) {    
    ...
    } else if (lhs_shape.ndim() == 2 && rhs_shape.ndim() == 2) {    
        if (lhs_shape[1] != rhs_shape[0]) {
            LONEXGRAD_EXCEPTION(
                std::format(
    ...
    } else if (lhs_shape.ndim() == 3 && rhs_shape.ndim() == 3) {    
...
auto prepare_multi_dim_reduce(const Tensor &input, std::vector<int> dims) -> std::pair<Tensor, std::vector<int>> {
    ...
    if (static_cast<int>(dims.size()) != N_old) {
        LONEXGRAD_EXCEPTION("Given dim list contains no-unique values");
    }
...
#define DECLARE_REDUCTION_MULTI_DIM(FUNC)                                                   \
    ...
    auto Tensor::FUNC(const std::vector<int> &dims, bool keep_dim) const -> Tensor {        \
        CHECK_VALID_TENSOR(*this);                                                          \
        return lonexgrad::FUNC(*this, dims, keep_dim);                                      \
    }
...
auto Tensor::var(int dim, bool keep_dim, int correction) const -> Tensor {
    CHECK_VALID_TENSOR(*this);
    CHECK_VALID_INDEX_EXCLUSIVE(dim, shape_.ndim());
    return lonexgrad::var(*this, dim, keep_dim, correction);
}
auto Tensor::var(const std::vector<int> &dims, bool keep_dim, int correction) const -> Tensor {
    CHECK_VALID_TENSOR(*this);
    return lonexgrad::var(*this, dims, keep_dim, correction);
}
auto Tensor::var(bool keep_dim, int correction) const -> Tensor {
    CHECK_VALID_TENSOR(*this);
    return lonexgrad::var(*this, keep_dim, correction);
}
...
#define DECLARE_BOOL_UNARY_OP(FUNC)                             \
    auto FUNC(const Tensor &tensor) -> Tensor {                 \
        ...
        if (tensor.dtype() != kBool) {                          \
            LONEXGRAD_EXCEPTION("Only boolean tensors are supported"); \
        }                                                       \
...
#define DECLARE_UNARY_INPLACE_OP(FUNC)                                         \
    auto Tensor::FUNC##_()->Tensor & {                                         \
        ...
        if (unary_conversion_required<common::unary::UnaryOpT::FUNC>(*this)) { \
            LONEXGRAD_EXCEPTION("Inplace abs requires dtype conversion.");             \
        }                                                                      \
        get_backend(this->device())->FUNC##_(*this);                           \
        ++version_count_;                                                      \
        return *this;                                                          \
...
auto Tensor::softplus_(double beta, double threshold) -> Tensor & {
    ...
    if (unary_conversion_required<common::unary::UnaryOpT::softplus>(*this)) {
        LONEXGRAD_EXCEPTION("Inplace softplus requires dtype conversion.");
    }
...
auto Tensor::leaky_relu_(double negative_slope) -> Tensor & {
    ...
    if (unary_conversion_required<common::unary::UnaryOpT::leaky_relu>(*this)) {
        LONEXGRAD_EXCEPTION("Inplace leaky_relu requires dtype conversion.");
    }
...
auto Tensor::elu_(double alpha) -> Tensor & {
    ...
    if (unary_conversion_required<common::unary::UnaryOpT::elu>(*this)) {
        LONEXGRAD_EXCEPTION("Inplace elu requires dtype conversion.");
    }
...
auto Tensor::hardtanh_(double min, double max) -> Tensor & {
    ...
    if (unary_conversion_required<common::unary::UnaryOpT::hardtanh>(*this)) {
        LONEXGRAD_EXCEPTION("Inplace hardtanh requires dtype conversion.");
    }
...
auto Tensor::softmax_(int dim) -> Tensor & {
    ...
    if (!is_float_dtype(scalar_type_)) {
        LONEXGRAD_EXCEPTION("Inplace softmax requires dtype conversion.");
    }
    CHECK_VALID_INDEX_EXCLUSIVE(dim, shape_.ndim());
    dim = indexing_exclusive(dim, shape_);
...
auto Tensor::log_softmax_(int dim) -> Tensor & {
    ...
    DISABLE_BOOL(*this, softmax_);
    if (!is_float_dtype(scalar_type_)) {
        LONEXGRAD_EXCEPTION("Inplace log_softmax requires dtype conversion.");
    }
    CHECK_VALID_INDEX_EXCLUSIVE(dim, shape_.ndim());
    dim = indexing_exclusive(dim, shape_);
...
auto where(const Tensor &cond, const Tensor &lhs, const Tensor &rhs) -> Tensor {
    ...
    if (cond.dtype() != kBool) {
        LONEXGRAD_EXCEPTION(std::format("Condition type needs to be kBool, given {:s}", cond.dtype()));
    }
...
auto isclose(const Tensor &lhs, const Tensor &rhs, const CloseOptions &options) -> Tensor {
    ...
    CHECK_SAME_SHAPE(lhs, rhs);
    if (options.atol() < 0) {
        LONEXGRAD_EXCEPTION("atol should be greater than or equal to 0.");
    }
...
[[nodiscard]] auto
    ...
    if (input.dim() != 4) {
        LONEXGRAD_EXCEPTION(std::format("Input shape {:s} expected to have 4 dimensions.", input.shape().to_string()));
    }
    if (weight.dim() != 4) {
        LONEXGRAD_EXCEPTION(std::format("Weight shape {:s} expected to have 4 dimensions.", weight.shape().to_string()));
    }
    ...
    if (stride <= 0) {
        LONEXGRAD_EXCEPTION(std::format("Stide of {:d} is expected to be positive.", stride));
    }
    if (padding < 0) {
        LONEXGRAD_EXCEPTION(std::format("Padding of {:d} is expected to be non-negative.", stride));
    }
...
auto embedding(const Tensor &input, const Tensor &weight) -> Tensor {
    if (!is_integral_dtype(input.dtype())) {
        LONEXGRAD_EXCEPTION(std::format("Expected input to be integral dtype, given {:s}", input.dtype()));
    }
    if (weight.dim() != 2) {
        LONEXGRAD_EXCEPTION(std::format("Expected weight to have 2 dimensions, given shape {:s}", weight.shape()));
    }
