...
TEST_CASE("Conv2D Forward") {
    auto test1 = []<typename T>(Device device) {
        ...
        CHECK(allclose(lonexgrad::conv2d(X, W, {}, S, P), expected));
    };
...
TEST_CASE("Conv2D Forward Torch") {
    auto test1 = []<typename T>(Device device) {
        ...
        CHECK(allclose(lonexgrad::conv2d(X, W, BIAS, S, P), expected, CloseOptions().atol(1e-6)));
    };
...
TEST_CASE("Max Pool2D Forward Torch") {
    auto test1 = []<typename T>(Device device) {
        ...
        CHECK(allclose(lonexgrad::max_pool2d(X, K, S, P), expected));
    };
    runner_single_type<float>(test1);
}
...
TEST_CASE("Min Pool2D Forward Torch") {
    auto test1 = []<typename T>(Device device) {
        ...
        Tensor expected(expected_torch_data, {B, C, H_out, W_out}, device);
        CHECK(allclose(lonexgrad::min_pool2d(X, K, S, P), expected));
    };
    runner_single_type<float>(test1);
}
...
TEST_CASE("Avg Pool2D Forward Torch") {
    auto test1 = []<typename T>(Device device) {
        ...
        Tensor expected(expected_torch_data, {B, C, H_out, W_out}, device);
        CHECK(allclose(lonexgrad::avg_pool2d(X, K, S, P), expected, CloseOptions().atol(1e-6)));
    };
    runner_single_type<float>(test1);
}
