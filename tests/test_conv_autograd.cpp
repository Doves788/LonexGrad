...
TEST_CASE("Conv2D autograd") {
    auto test1 = []<typename T>(Device device) {
        ...
        Tensor out = lonexgrad::conv2d(X, W, BIAS, S, P);
        out.backward(grad);
...
TEST_CASE("Max pool autograd") {
    auto test1 = []<typename T>(Device device) {
        ...
        Tensor out = lonexgrad::max_pool2d(X, K, S, P);
        out.backward(grad);
...
TEST_CASE("Min pool autograd") {
    auto test1 = []<typename T>(Device device) {
        ...
        Tensor X(_X, {B, C, H_in, W_in}, device, true);
        Tensor grad(_grad, {B, C, H_out, W_out}, device);
        Tensor out = lonexgrad::min_pool2d(X, K, S, P);
        out.backward(grad);
...
TEST_CASE("Avg pool autograd") {
    auto test1 = []<typename T>(Device device) {
        ...
        Tensor X(_X, {B, C, H_in, W_in}, device, true);
        Tensor grad(_grad, {B, C, H_out, W_out}, device);
        Tensor out = lonexgrad::avg_pool2d(X, K, S, P);
        out.backward(grad);
